...
int main()
{
    ...
    while(std::getline(std::cin, string)) {
        ...
        if ( line != 0) {
            stringsBar.at(line) += stringsBar.at(line - 1);
        }
        ...
        if (i > pattern.size() * 5) {
           ...
           // }
            int possibleEntry = zAlgorithm->matchPattern(text, pattern, stringsBar, result, lastString, lastAlpha);
    ...
    if (line != 0) {
        ...
        zAlgorithm->matchPattern(text,pattern,stringsBar, result, lastString,lastAlpha);
    }
