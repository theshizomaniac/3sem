...
typedef struct SubStringSearchResult Res;
...
std::vector<Res> subMatch(1);



int doTraversal(Node *n, int labelHeight, int* maxHeight, 
int* substringStartIndex, std::vector <int> &matchIndex)
{
    if(n == NULL)
    {
        return 0;
        ...
        for (i = 0; i < MAX_CHAR; i++)
        {
            if(n->children[i] != NULL)
            {
                ret = doTraversal(n->children[i], labelHeight + 
                    edgeLength(n->children[i]), 
                    maxHeight, substringStartIndex, matchIndex);
                    ...
                    n->suffixIndex == -4)
                {
                    ...
                    if(*maxHeight < labelHeight)
                    {
                        ...
                        *substringStartIndex = *(n->end) - 
                            labelHeight + 1;
                        std::cout <<"match? "<< *(n->end) -
                                                labelHeight +1 << std::endl;
                        /*subMatch.clear();
                        Res tmp;
                        tmp.index = *(n->end) - labelHeight +1;
                        tmp.length = *maxHeight;
                        subMatch.push_back(tmp);*/
                        matchIndex.clear();
                        matchIndex.push_back(*(n->end) - labelHeight +1);
                    }
                       ...
                       /*Res tmp;
                        ...
                        subMatch.push_back(tmp);*/
                        matchIndex.push_back(*(n->end) - labelHeight +1);
                        std::cout << "same match" << *(n->end) - labelHeight +1 << std::endl;
                    }
                }
            }
        }
...
bool CheckNoRepeat(int first, int second, int maxHeight)
{
    ...
    for (int i = 0; i < maxHeight; ++i) {
        if (text[i + second] == text[i + first]) {
            return false;
        }
        std::cout << text[i + second] << text[i + first] << std::endl;
    }
    return true;
}
...
void getLongestCommonSubstring()
{
    ...
    doTraversal(root, 0, &maxHeight, &substringStartIndex, matchIndex);
    /*for (auto i: subMatch) {

            int k;
            for (k=0; k<maxHeight; k++)
                printf("%c", text[k + i.index]);
            if(k == 0)
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    cont[size1] = '#';
    printf("\n%s\n", cont);
    for (int i = size1 + 1; i < size1 + strlen(b) + 1; ++i) {
        cont[i] = b[i - size1-1];
    }
    cont[size1 + strlen(b) + 1] = '$';
    printf("\n%s\n", cont);*/
