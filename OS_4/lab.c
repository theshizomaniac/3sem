...
void SearchAll (const char * pattern, bool check_case)
{
    ...
    int prev_i = 0;
    last_string = 1;
    for (int i = MAX_MMAP_SIZE; i < sb.st_size; i += MAX_MMAP_SIZE) {
...
void WriteToFile (const char * string, int pos)
{
    ...
    Mmap_part(sb.st_size - MAX_MMAP_SIZE, sb.st_size,false);
    int x = MAX_MMAP_SIZE;
    for (int i = sb.st_size; i >= pos + z; --i) {
        if (x == 0) {
            Munmap_part();
            Mmap_part(i - MAX_MMAP_SIZE,i + z,false);
...
void DeleteChars(int a, int b)
{
    int z = b- a;
    int pos = a;
    Mmap_part(pos, pos+MAX_MMAP_SIZE,false);
    int x = MAX_MMAP_SIZE;
    for (int i = pos; i < sb.st_size - z; ++i) {
        if (x == 1) {
            Munmap_part();
            Mmap_part(i,i + MAX_MMAP_SIZE,false);
            x = MAX_MMAP_SIZE;
        }
        addr[i] = addr[i + z];
        x--;
...
void Count ()
{
    ...
    int file_strings = 1;
    Mmap_part(0,MAX_MMAP_SIZE,true);
    for (int i = 0; i < sb.st_size; ++i) {
        if (file_size_check % MAX_MMAP_SIZE == 0) {
            Munmap_part();
            Mmap_part(i, i + MAX_MMAP_SIZE, true);
        }
        file_size_check++;
        if(addr[i] == '\n') {
            file_strings++;
